...
def create_treatment(
    ...
    patient = (
        db.query(PatientModel).filter(PatientModel.id == treatment.patient_id).first()
    )
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")

    db_treatment = TreatmentModel(
        **treatment.dict(), created_by_user_id=current_user.id
    )
    db.add(db_treatment)
    db.commit()
...
def read_treatment(
    ...
    treatment = (
        db.query(TreatmentModel).filter(TreatmentModel.id == treatment_id).first()
    )
...
@router.put("/treatments/{treatment_id}/suspend", response_model=Treatment)
def suspend_treatment(
    treatment_id: str,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    ...
    # When suspending, mark as finished and update end date to today
    setattr(db_treatment, "status", "finished")
    setattr(db_treatment, "programmed_days", db_treatment.days_applied)
    db.commit()
    db.refresh(db_treatment)
    return db_treatment
